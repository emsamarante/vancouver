...
datetime_series = pd.to_datetime(df[['YEAR', 'MONTH', 'DAY', 'HOUR', 'MINUTE']])
...
layout = dbc.Container(children=[
    ...
    dbc.Row([
        dbc.Col([
            dbc.Row([
                ...
                dbc.Col([
                    dcc.Dropdown(
                        id="drop-month-1",
                        value = 4,
                        options=[{'label': key, 'value': value} for key, value in zip(months.keys(), months.values())],
                        clearable=False
                    )
                ], lg=3),
                dbc.Col([
            ...
            dbc.Row([
                dbc.Col([
                    CARD("Total of Crimes by month", "id-card-indicator-2", lg=12),
                    make_tooltip("Comparison with previous year by type of crime", "id-card-indicator-2")
                ], lg=3),
                dbc.Col([
                    CARD("Total of Crimes", "id-card-indicator-1", lg=12),
                    make_tooltip("Comparison with previous year", "id-card-indicator-1")
                ], lg=3),
                dbc.Col([
                    CARD("Crime by neighbourhood", "id-graph-1", lg=12),
                    dbc.Tooltip(
                ...
                dbc.Col([
                    ...
                    dbc.Row([
                        dbc.Col([
                            ...
                            make_tooltip("Seeing the crimes over year", "id-graph-line-1")
                        ]),
                    ], className='g-2 my-auto'),                
                ], lg=6),
                dbc.Col([
                    dbc.Row([
                        ...
                        dbc.Col([
                            dcc.Dropdown(
                                id="drop-bairro-1",
                                value="Fairview",
                                options=[{'label':i, "value":i} for i in df.NEIGHBOURHOOD.unique()],
                            )
                        ],lg=6),
                        dbc.Col([
                            dcc.Dropdown(
                    ...
                    dbc.Row([
                        dbc.Col([
                            CARD("Direct Comparisson", "id-graph-comparison", lg=12),
            ...
            dbc.Row([
                dbc.Col([CARD("Análise por estações", "id-estacao-graph", lg=12)],lg=8),
                dbc.Col([CARD("Análise por período", "id-periodo", lg=12)], lg=4)
            ], className='g-2 my-auto')
...
#     df_crimes = df.groupby([pd.PeriodIndex(df['DATE'], freq="M"), 'YEAR', 'MONTH','TYPE'])['HUNDRED_BLOCK'].count().reset_index().rename(columns={'HUNDRED_BLOCK':'COUNTING'})
#     dff = df_crimes[(df_crimes.YEAR.isin([year])) & ((df_crimes.MONTH.isin([month])) | (df_crimes.MONTH.isin([month-1])))]
...
#         delta = {'relative': True, 'valueformat': '.1%', 'reference': df_filtered.at[df_filtered.index[0],'COUNTING']}
#     ))
...
#     fig.update_layout(main_config, height=230, template=template)
...
#     aux = dff.groupby(['YEAR']).count()['DAY'].reset_index().rename(columns={'DAY': 'COUNTING'})
...
#         number = {'valueformat': '.0f'},
#         delta = {'relative': True, 'valueformat': '.1%', 'reference': aux[aux.YEAR.isin([year-1])]['COUNTING'].values[0]}
#     ))

#     del aux

#     fig_indicator.update_layout(main_config, height=230, template=template)
...
#     aux = dff.groupby(['NEIGHBOURHOOD']).count()['DAY'].reset_index().rename(columns={'DAY': 'COUNTING'})
#     aux = aux.sort_values(['COUNTING', 'NEIGHBOURHOOD'], ascending=False).reset_index()
#     aux1 = aux.copy()

#     aux1['NEIGHBOURHOOD'] = np.where(aux1.loc[:, "COUNTING"] < aux1.at[aux1.index[4], "COUNTING"], "Others Neighbourhoods", aux1['NEIGHBOURHOOD'])
#     aux1 = aux1.groupby(['NEIGHBOURHOOD'])['COUNTING'].sum().reset_index().sort_values(['COUNTING'])
...
#     fig_bar.update_layout(main_config, height=230, template=template, yaxis_title=None)
...
#     df = pd.DataFrame(data)
#     df = df.groupby(['TYPE','YEAR'])['DAY'].count().reset_index().rename(columns={'DAY':'COUNTING'})
...
#     # updates
#     fig.update_layout(main_config, height=250, xaxis_title=None)
...
#     df_bairro1 = df1.groupby(pd.PeriodIndex(df1['DATA'], freq="M"))['DAY'].count().reset_index().rename(columns={'DAY':'COUNTING'})
#     df_bairro2 = df2.groupby(pd.PeriodIndex(df2['DATA'], freq="M"))['DAY'].count().reset_index().rename(columns={'DAY':'COUNTING'})
...
#     # Abaixo de zero
#     fig.add_scattergl(name=bairro2, x=df_final['DATA'], y=df_final['COUNTING'].where(df_final['COUNTING'] > 0.00000))
#     # Updates
...
def update_graph(data, crime, toggle):
    ...
    fig = px.pie(dff.groupby(['HOUR', 'PERIOD'])['DAY'].count().reset_index().rename(columns={'DAY':'COUNTING'}),
...
def update_graph(data, crime, year, toggle):
    ...
    fig = px.bar(dff.groupby('NEIGHBOURHOOD')['DAY'].count().reset_index().rename(columns={'DAY':'COUNTING'}),
             x = 'NEIGHBOURHOOD', values='COUNTING', color='PERIOD')
